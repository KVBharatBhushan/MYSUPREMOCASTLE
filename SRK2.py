...
for found_row in found_rows:
    ...
    above_rows = [['R' + str(row["S.No"]), row["Value"]] for row in above_rows]

    # Row matching the specified Value
    cur_row = ['R' + str(found_row["S.No"]), found_row["Value"]]

    # Rows below
    below_rows = table[index + 1:index + 3]
    below_rows = [['R' + str(row["S.No"]), row["Value"]] for row in below_rows]

    # Add the rows to the PrettyTable
    x.add_row([above_rows, cur_row, below_rows])
