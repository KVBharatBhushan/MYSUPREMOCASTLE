...
# Function to create a dictionary for each row
cr = lambda s_no, value: {"S.No": s_no, "Value": value}
...
target_value = int(input("Enter Input Number:"))
...
for found_row in found_rows:
    ...
    above_rows = [row["Value"] for row in above_rows]

    # Row matching the specified Value
    #cur_row = [str(found_row["S.No"]), found_row["Value"]]

    # Rows below
    below_rows = table[index + 1:index + 3]
    below_rows = [row["Value"] for row in below_rows]

    # Add the rows to the PrettyTable
    x.add_row([above_rows,below_rows])
